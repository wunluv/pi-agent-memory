...
| `memory_search(query)` | BM25 ranked with importance/recency boosts. Leads with the scope it searched. Fans out over that root **and** the shared `~/.pi/org/insights/` commons in one ranking pass; hits grouped by corpus, shared hits attributed by author (`corpus` selects `local` \| `shared` \| `all`). |
...
- **The shared commons is written with a bare `insights/…` path.** `memory_write("insights/tooling/x.md", …)` lands in `~/.pi/org/insights/` and rides the org repo's sync, whatever project the session is bound to. Reads and tree listings take the same prefix. An explicit `root` still overrides it, which is how you reach a project's own `insights/` if one exists (the result warns when both do).
