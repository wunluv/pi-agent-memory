...
/** Provision a Zone B remote when a project is initialized or first committed. */
function provisionProjectRemote(root: string): void {
	const config = loadSyncConfig(syncEnv);
	...
	try {
		const remote = projectRepoUrl(config.server_url, projectName);
		if (isPrivateMemoryRemote(config.server_url, remote)) provisionRemote(syncEnv, remote);
	} catch {
		// Invalid project names never become sync targets.
	}
}
	...
	pi.registerCommand("startwork", {
		...
		handler: async (args, ctx) => {
			...
			if (choice === "yes") {
				...
				syncOrgAfterWrite();
				provisionProjectRemote(result.memoryPath);
