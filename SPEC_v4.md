...
- **Creation always lands on `ephemeral`** (`identity.ts:348` is the hardcoded default; nothing auto-promotes, since promotion is human-gated). The trial ends by hand: `/agent:promote <name>`. Because the state is silent, every surface that creates or displays it must name it and name the exit — the backfill notify, `memory_status`, and `/agent:init` (#67).
- **Promotion policy (San, 2026-09-13):** ephemeral = a new team member under trial. Promote once synergy is proven. The state is a roster label only; no code path gates sync, context, writes, or recall on it.
...
## Future Direction (Phase 3+, designed 2026-08-20)

San is rebuilding MSA (Med Student Advisors) to be 99% agent-powered, scaling to 1000s of customers, each with a concierge agent; Context Institute's landing-page factory is the same multi-tenant pattern. This section records the direction so the Phase 2 foundation is built to compose with it. See `.memory/reference/pi-agent-memory/future-direction-2026-08-20.md`.

- **Soul-host daemon** — an always-on process (regen) that inhabits souls when no terminal is open. Listens (Telegram + event webhooks), routes `chat_id/event → uuid`, runs sessions (same pull/run/commit/push contract as pi), and initiates messages (proactive agents). One daemon = one writer per soul, resolving concurrency cleanly.
- **Three interaction patterns** — personal access (San→Alph on mobile), product agents (multi-tenant: customer = soul = uuid = bare repo), proactive agents (agent-initiated, e.g. Heavenletters CRM donation alerts with an Approve/Edit/Regenerate approval loop — each approval is a memory-write training signal).
- **Multi-tenancy is free** — N customers = N UUID-named bare repos (KBs each). The security boundary lives in the daemon (trusted broker); customers never touch git directly (no SSH keys), they authenticate via chat identity.
- **Interfaces** — Telegram first; WhatsApp/Signal on demand; web dashboard; email (agents hold their own email accounts). A per-customer **assets directory** holds documents/files that land via any channel (binary, distinct from markdown memory).
- **Concurrency model** — optimistic concurrency control via git: every write is a commit on HEAD; non-fast-forward push rejected; `pull --rebase --autostash`, never force; same-file conflict → abort, human resolves. See `.memory/reference/pi-agent-memory/concurrency-model-2026-08-20.md`.
- **Foundation principle** — build the substrate (#7 done, #8 sync next, #24 multi-soul) so the daemon is a thin client, not a fork. Product systems (LP factory, MSA concierge, CRM) are designed later, not now.
