...
| B — Project | `<project>/.memory/` | Local git only, no remote | Strategy, per-project status, decisions, observations, project_insights | `/startwork` pivots here |
...
└── memory/                          ← git repo (one per agent)
    ...
    │
    ├── reference/                   ← LAZY: semantic wiki-paths
    │   └── <project>/               # Cross-project knowledge that doesn't belong in Zone B
    │
    └── _meta/                       ← LAZY: cross-project concerns
        ├── observations/
        ├── feedback/
        └── decisions/
```
...
```
~/Projects/BTTN/
  .memory/                          ← local git repo (NO remote)
    reference/
...
- Server sync uses Letta memfs or custom git HTTP backend
...
12. **Zone B `.memory/` has no git remote** — local version control only
